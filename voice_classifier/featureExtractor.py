...
def homogenize_format(gender, age_range, pronunciation):
    ...
    if british_pattern.search(pronunciation):
        pronunciation = 'British'
    elif american_pattern.search(pronunciation):
        pronunciation = 'American'
    elif european_pattern.search(pronunciation):
        pronunciation = 'European'
    elif indian_pattern.search(pronunciation):
        pronunciation = 'Indian'
    elif australian_pattern.search(pronunciation):
...
def get_frequencies(sample_wav_folder):
    # extract list of dominant frequencies in sliding windows of duration defined by 'step' for each of the 10 wav
    # files and return an array
    ...
    for wav_file in os.listdir(sample_wav_folder):
        ...
        for i in range(0, len(data), step):
            ...
            filtered_frequencies = [f for f in window_frequencies if
                                    20 < f < 280 and not 46 < f < 66]  # I see noise at 50Hz and 60Hz
...
def main():
    ...
    for i in range(n_samples):
        ...
        if os.path.isfile(readme_file):
            ...
            print(gender, age_range, pronunciation)
        ...
        if os.path.isdir(
                sample_wav_folder):  # some of the samples don't contain a wav folder (Ex: 'LunaTick-20080329-vf1')
            frequencies = get_frequencies(sample_wav_folder)
            if len(frequencies) > 10:
                # for some of the files (ex: Aaron-20130527-giy) I only recover frequencies of 0.0 (even if I don't
                # split in chunks) which is not integrated into my lol and frequencies is empty
                ...
                df.loc[i] = pd.Series(sample_dict)

    df.to_csv('voice-data.csv')


def get_features_chunk(chunks):
    # create dataframe
    columns = ['nobs', 'mean', 'skew', 'kurtosis', 'median', 'mode', 'std', 'low', 'peak', 'q25', 'q75', 'iqr']
    df = pd.DataFrame(columns=columns)
    ...
    for chunk in chunks:
        # get features
        nobs, mean, skew, kurtosis, median, mode, std, low, peak, q25, q75, iqr = get_features(chunk)
        # to dataframe
        sample_dict = {'nobs': nobs, 'mean': mean, 'skew': skew, 'kurtosis': kurtosis,
                       'median': median, 'mode': mode, 'std': std, 'low': low,
                       'peak': peak, 'q25': q25, 'q75': q75, 'iqr': iqr}
